  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        if (user?.email) {
          const userRankIndex = usersList.findIndex((entry) => entry.email === user.email);
          setUserRank(userRankIndex >= 0 ? userRankIndex + 1 : null);
        }
      } catch (error) {
        console.error("Failed to load leaderboard:", error);
        showError(error, "Could not load the leaderboard. Please try again.");
      }
    };
      ...
      <div className="w-full max-w-4xl text-white mx-auto p-2 md:p-10 h-screen">
        ...
        {user && userRank && (
          <div className="mt-6 text-lg text-white">
            <p className="text-center font-bold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">
              Your Rank: <span className="text-white">#{userRank}</span>
            </p>
            <div className="mt-4">
